...
describe('Unit test for TableCellFactory', function () {
    beforeEach(function () {
    ...
    test('createTableCell is called successfully with all type of cell', function () {
        try {
            var typeList = ['dropdown', 'multichoice', 'radio', 'label', 'icon', 'alert'];
            ...
            var myTable = new Table({
                ...
                defaultRowData: {},
                data: [componentProps_1],
                columns: typeList.map(function (type, index) {
                    return {
                        header: type,
                        cell: function () {
                            return createTableCell(type, type, componentProps_1[type]);
                        }
                    };
                })
            });
    ...
    test('createTableCell listener work normally', function () {
        var typeList = ['text', 'radio', 'label'];
        var newTextValue = 'New text value';
        var newRadioValue = 'Orange';
        var componentProps = {
            text: {
                value: 'Text value',
        ...
        var myTable = new Table({
            ...
            defaultRowData: {},
            data: [componentProps],
            columns: typeList.map(function (type) {
                return {
                    header: type,
                    cell: function () {
        ...
        var labels = container.getElementsByClassName('kuc-label');
        fireEvent.click(labels[0]);
        // TODO: Remove unreachable case line 103 TableCellFactory.ts
    });
    test('createTableCell throw error when called with invalid type of cell', function () {
        try {
    ...
    test('createTableCell throw error when called without fieldname', function () {
        try {
            // @ts-ignore
